...
public class ElevatorTests implements MockitoTest, EventTest {
    ...
    @Test
    @DisplayName("can schedule multiple floor stops")
    void moveBetweenMultipleFloors() {
        // when: requests are made for various floors
        sut.moveToFloor(3);
        sut.moveToFloor(10);
        
        // then: the elevator publishes its events
        List<ElevatorOperationEvent> expectedEvents = captureEvents((e) -> Arrays.asList(
                    ElevatorMovingEvent.class,
                    ElevatorFloorPassEvent.class, 
                    ElevatorFloorStopEvent.class, 
                    ElevatorDoorOpenEvent.class, 
                    ElevatorDoorCloseEvent.class
        ).stream()
                
                .anyMatch((it) -> it.isAssignableFrom(e.getClass())), 22);
        
        // and: the events are in the correct order
        int eventIdx = 0;
        
        // starts moving to floor 2
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorMovingEvent);
        assertEquals(ElevatorMovingEvent.Direction.UP, ((ElevatorMovingEvent)expectedEvents.get(eventIdx)).getDirection());
        ++eventIdx;
        
        // passes floor 2
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorFloorPassEvent);
        assertEquals(2, ((ElevatorFloorPassEvent)expectedEvents.get(eventIdx)).getFloor());      
        ++eventIdx;
        ...
        // stops at floor 3
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorFloorStopEvent);
        assertEquals(3, ((ElevatorFloorStopEvent)expectedEvents.get(eventIdx)).getFloor()); 
        ++eventIdx;
        
        // opens doors at floor 3
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorDoorOpenEvent);
        assertEquals(3, ((ElevatorDoorOpenEvent)expectedEvents.get(eventIdx)).getFloor());  
        ++eventIdx;
        
        // closes doors at floor 3
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorDoorCloseEvent);
        assertEquals(3, ((ElevatorDoorCloseEvent)expectedEvents.get(eventIdx)).getFloor());  
        ++eventIdx;   
        
        // moves and passes floors 4-8
        for (Integer floor : Arrays.asList(4, 5, 6, 7, 8, 9)) {
            // moving to floor
            assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorMovingEvent);
            assertEquals(ElevatorMovingEvent.Direction.UP, ((ElevatorMovingEvent) expectedEvents.get(eventIdx)).getDirection());
            ++eventIdx;
            ...
            // passes floor
            assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorFloorPassEvent);
            assertEquals(floor.intValue(), ((ElevatorFloorPassEvent)expectedEvents.get(eventIdx)).getFloor());      
            ++eventIdx; 
        }      
        ...
        // stops at floor 10
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorFloorStopEvent);
        assertEquals(10, ((ElevatorFloorStopEvent)expectedEvents.get(eventIdx)).getFloor()); 
        ++eventIdx;

        // opens doors at floor 10
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorDoorOpenEvent);
        assertEquals(10, ((ElevatorDoorOpenEvent)expectedEvents.get(eventIdx)).getFloor());  
        ++eventIdx;

        // closes doors at floor 10
        assertTrue(expectedEvents.get(eventIdx) instanceof ElevatorDoorCloseEvent);
        assertEquals(10, ((ElevatorDoorCloseEvent)expectedEvents.get(eventIdx)).getFloor());  
        ++eventIdx;   
        
        
        // and: all the events report for the correct elevator
        assertTrue(expectedEvents.stream().allMatch((e) -> e.getElevator().getId().equals(sut.describe().getId())), 
                "All events should identify the elevator.");
    }
